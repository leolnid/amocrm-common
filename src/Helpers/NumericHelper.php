...
class NumericHelper
{
    public static function spellPrice(float $value): string
    {
        return self::num2str($value, 1);
    }

    public static function spell(float $value)
    {
        return self::num2str($value);
    }

    //TODO Переписать это дерьмо
    public static function num2str($num, $rubles = false)
    {
        $nul = 'ноль';
        $ten = [
        ...
        if (! $rubles) {
            return collect($out)->reject('')->implode(' ');
        }
